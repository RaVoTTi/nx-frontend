...
export class CheckoutService {
  ...
  getPreOrder(id: string): Observable<IResponse<IOrder>> {
    return this.http.get<IResponse<IOrder>>(`${this.API_URL}/order/preorder/${id}`);
  }
  patchSendPayment(orderId: string, token: string ):  Observable<IResponse<any>> {
    return this.http.patch<IResponse<any>>(`${this.API_URL}/order/checkout/${orderId}`, {token}) ;
